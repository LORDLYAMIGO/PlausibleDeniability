...
program.command('add').argument('<container>').argument('<input>').option('--password-env <name>').action(async (container, input, options) => { await addPayload(container, passwordFromEnv(options.passwordEnv), await fs.readFile(input), basename(input)); console.log('Payload added.'); });
